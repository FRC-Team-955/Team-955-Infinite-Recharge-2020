...
void ToggleMotor::Toggle(int toggle_motor_button, TalonSRX * talon_toggle) {
    if (joystick->GetRawButton(toggle_motor_button) == false && button_check == true) {
        ...
        talon_toggle->Set(ControlMode::PercentOutput, power); //power is set in robot.cpp-teleopinit
        motor_check = true;
        button_check = true;
